...
func handleAdminBotStart(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		sessionsMu.Lock()
		var session *Session
		// First pass: find a session with a LIVE WebSocket (user actively monitoring this room)
		for _, s := range sessions {
			s.mu.RLock()
			isSameRoom := s.room == req.Room
			isOrphaned := s.orphaned
			s.mu.RUnlock()
			if isSameRoom && !isOrphaned {
				s.wsMu.Lock()
				hasWS := len(s.wsSet) > 0
				s.wsMu.Unlock()
				if hasWS {
		...
		// Second pass: fall back to any session for this room (orphaned/headless)
		if session == nil {
			for _, s := range sessions {
				s.mu.RLock()
				if s.room == req.Room {
		...
		if session == nil {
			...
			session = &Session{
				id:          id,
				wsSet:       make(map[*websocket.Conn]bool),
